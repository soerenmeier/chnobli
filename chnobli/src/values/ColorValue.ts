...
export default class ColorValue {
	...
	static parse(v: string | number[]) {
		if (Array.isArray(v)) {
			...
			return new ColorValue(v as any);
		}
		...
		if (v.startsWith('#')) return new ColorValue(parseHex(v.substring(1)));
		if (v.startsWith('rgb')) return new ColorValue(parseRgb(v));
		if (v.startsWith('hsl')) return new ColorValue(parseHsl(v));

		if (v in COLORS) return new ColorValue(COLORS[v]);

		throw new Error('unknown color value ' + v);
	}

	// v needs to be a string
	static mightBeAColor(v: string) {
		v = v.trim();
	...
	clone() {
		return new ColorValue(this.values.slice() as any);
	}

	toString() {
		const [r, g, b, a] = this.values;
		return `rgba(${[r, g, b].map(v => Math.round(v)).join(', ')}, ${a.toFixed(
			3,
		)})`;
	}
}
